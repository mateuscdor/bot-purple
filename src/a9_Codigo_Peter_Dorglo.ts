...
const startSock = () => {
  ...
  if (moduleSuccess)
    console.log(
      chalk.green.bold(
        "[INFO] All Plugins Installed Successfully. The bot is ready to use."
      )
    );
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    if (BotsApp.isCmd) {
      console.log(
        chalk.redBright.bold(`[INFO] ${BotsApp.commandName} command received.`)
      );
      const command = commandHandler.get(BotsApp.commandName);
      const args = BotsApp.body.trim().split(/\s+/).slice(1);

      if (!command) {
        await TRANSMIT.sendMessageWTyping(sock, BotsApp.chat, {
