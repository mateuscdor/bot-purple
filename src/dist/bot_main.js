...
import P from "pino";
import makeWASocket, { DisconnectReason, delay, useSingleFileAuthState, } from "@adiwajshing/baileys-md";
        ...
        sock.ev.on("messages.upsert", (m) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!msg.key.fromMe && m.type === "notify") {
                ...
                if (Fs.exists(`./customerData/${accountID}.json`)) {
                    ...
                    const customerDataRaw = readFromFile(`./customerData/${accountID}.json`);
                    const customerData = JSON.parse(customerDataRaw);
                    log(`Configuração padrão do novo contato "${accountID}" criada.`);
                    log(customerData.accountID);
                    //send non newcomer message
                    const msgRandom = variableMessage(botMsgs.bemVindoClienteAntigo1, botMsgs.bemVindoClienteAntigo2, botMsgs.bemVindoClienteAntigo3, botMsgs.bemVindoClienteAntigo4);
                    const msgRandomObj = {
                        text: msgRandom,
                    };
                    yield sendMessageWTyping(accountID, msgRandomObj);
                    /*if (rand <= 25) {
                ...
                yield sock.sendReadReceipt(msg.key.remoteJid, msg.key.participant, [
                    msg.key.id,
                ]);
                yield sendMessageWTyping(msg.key.remoteJid, { text: "Hello there!" });
                // send a simple text!
                yield sock.sendMessage(msg.key.remoteJid, { text: "oh hello there" });
                ...
                //send the message.
                yield sock.sendMessage(msg.key.remoteJid, buttonMessage);
                // list message send:
                /*
